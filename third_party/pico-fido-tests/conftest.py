...
class Device():
    ...
    def __set_server(self, rp, attestation):
        ...
        # Host-side accommodation (like the macos.py timeout patch): mirror only
        # the getInfo algorithms this fido2 library can VERIFY — the firmware now
        # leads with ML-DSA-44 (-48), which python-fido2 parses as UnsupportedKey
        # and cannot check signatures for. The authenticator legitimately signs
        # with any algorithm the request offers, so don't offer what we can't verify.
        from fido2.cose import CoseKey as _CoseKey, UnsupportedKey as _UnsupportedKey
        self.__server.allowed_algorithms = [
            ...
            if _CoseKey.for_alg(p['alg']) is not _UnsupportedKey
        ]
